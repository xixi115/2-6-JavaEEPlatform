...
public class ProductController {
    ...
    public ReturnObject searchProductByName(@RequestParam String name, @RequestParam(required = false, defaultValue = "auto") String type) {
        ...
        if (null != type && "manual".equals(type)){
            ...
            data = productList.stream().map(o->CloneFactory.copy(new ProductDto(),o)).collect(Collectors.toList());

        } else if (null != type && "jpa".equals(type)) {
            List<ProductEntity> productByNameManualJpa = productService.findProductByName_jpa(name);
            data = productByNameManualJpa.stream().map(o->CloneFactory.copy(new ProductDto(),o)).collect(Collectors.toList());
        } else {
