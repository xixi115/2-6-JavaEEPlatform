...
//        logger.debug("retrieveProductByName: productList = {}", productList);
    ...
    public List<ProductEntity> findByName_jpa(String name) {
        LocalDateTime time = LocalDateTime.now();
        List<ProductEntity> productList = productRepository.findByName(name);
        for(ProductEntity product : productList){
            Long id = product.getId();
            List<OnsaleEntity> onSaleList = onsaleRepository
                    .findByProductIdAndBeginTimeBeforeAndEndTimeAfter(id,time,time);
            product.setOnSaleList(onSaleList);
            Long goods_id = product.getGoodsId();
            List<ProductEntity> otherProduct = productRepository.findByGoodsIdAndIdNot(goods_id,id);
            product.setOtherProduct(otherProduct);
        }
        return productList;
    }
